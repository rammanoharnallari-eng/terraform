locals {
  ...
  mongo_url = "mongodb://${var.mongo_app_user}:${var.mongo_app_password}@"
              + join(",", local.mongo_seed_hosts)
              + "/${var.mongo_app_database}?replicaSet=rs0&authSource=${var.mongo_app_database}"
}

resource "kubernetes_secret" "app_conn" {
  metadata { name = "app-conn"; namespace = var.namespace }
